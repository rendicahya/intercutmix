...
This step will take several hours and the resulting videos will be stored in `data/ucf101/xgtf/scene`.
...
The output will be stored in `data/ucf101/xgtf/scene-list.json`.
...
# Symlink:
# mkdir data/ucf101/UniDet
# ln -s /nas.dbms/randy/datasets/ucf101/UniDet/json data/ucf101/UniDet/json
```

Output videos can be generated by setting `unidet.detect.output.video.generate` to `true`. Then the generated videos will be saved in `data/ucf101/UniDet/videos`.

5. Filter object detection.

This will select only relevant objects based on the relevancy between the video class names and the detected object names (step F). The output are `.pckl` files stored in `data/ucf101/UniDet/select/inter/dump`.

```shell
python batch-select.py
```

There are two modes: `actorcutmix` and `intercutmix` (default) that can be configured in `unidet.select.mode`. If you change it, make sure to change `unidet.select.output.video.path`, `unidet.select.output.mask.path`, and `unidet.select.output.dump.path` accordingly.
